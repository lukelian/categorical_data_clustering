function [cid]=Correct_Hard_K_Mode(Data,K)
...
[n,d] = size(Data);   %获得数据集的对象个数n、属性个数d
 ...
 while 1==1
        InitialCentersno=fix(rand(1,K)*(n-1)+1);
        if (length(unique(InitialCentersno))==K) &&((size((unique(Data(InitialCentersno,:),'rows')),1))==K)                  %错误多余：(size((unique(Data(InitialCentersno,:),'row')),1))==K
            break;
        end
 end
InitialCentersno   %输出初始类中心的序号

cid = zeros(1,n);   % cid保存聚类之后各个对象所属类标签

maxgn= 100;    %设置算法迭代的最大次数
iter = 0;
