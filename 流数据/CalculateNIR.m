function [weightingCell,column,k,attributeFrequence] = CalculateNIR(labelCollection,data)
...
for i = 1:1:k
    for j = 1:1:c
        ...
        for l = 1:1:attributeValueNum
            pSave{i}(j,l) = clusterCell{i}{j,1}(1,l)/attributeFrequence(j,l);
        end
    end
end
...
for i = 1:1:k
    for j = 1:1:c
        ...
        for l = 1:1:attributeValueNum
            if pSave{i}(j,l)~=0
                weightingFun(j,l) = (-1/log2(k)*(pSave{i}(j,l)*log2(pSave{i}(j,l)))) + weightingFun(j,l);
            end
        end
    end
end
